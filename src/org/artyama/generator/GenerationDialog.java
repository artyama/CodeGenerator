 ...
 */
package org.artyama.generator;

import org.apache.log4j.Logger;
import org.artyama.config.Configuration;
import org.artyama.config.model.DatabaseElement;
import org.artyama.config.model.TemplateElement;
import org.artyama.db.model.Table;
import org.artyama.generator.engine.EngineBuilder;
import org.artyama.generator.engine.TemplateEngine;
import org.artyama.generator.mapper.PaginationPlugin;
import org.artyama.generator.ui.component.CheckBoxList;
import org.artyama.util.StringUtil;
...
public class GenerationDialog extends JDialog {
    ...
    public GenerationDialog(Configuration configuration, Table tableModel, String classPath){
        ...
        lblTargetProject.setVisible(false);
        ...
        textTargetProject.setVisible(false);
    ...
    private void processSelectedTemplates(Object[] selectedTemplateElements, String targetProject, String basePackage, String moduleName, String tableAlias){
        ...
        Iterator it = configuration.getConnectionHistory().iterator();
        DatabaseElement first =(DatabaseElement)it.next();
        model.put("connectionUrl", first.getConnectionUrl());
        model.put("password", first.getPassword());
        model.put("username", first.getUsername());
        model.put("jypath", "${jypath}");

        for (TemplateElement templateElement : configuration.getTemplates()) {
            templateElement.setSelected(false);
        }

        TemplateElement templateElement_Dao = null;
        ...
        for (int i=0;i<count; i++) {
            try {
                ...
                else if(templateElement.getTemplateName().equals("BaseDao")){
                	templateElement_Dao = templateElement;
                }
                else {
        ...
        configuration.save();
        PaginationPlugin mapperPlugin = new PaginationPlugin();
        mapperPlugin.generate();
        try {
        	templateElement_Dao.setSelected(true);

            TemplateEngine templateEngine = engineBuilder.getTemplateEngine(templateElement_Dao.getEngine());
            if(templateEngine==null){
                JOptionPane.showMessageDialog(this, "没有对应的模板引擎: "+templateElement_Dao.getEngine(), "错误", JOptionPane.ERROR_MESSAGE);
            }
            else if(templateElement_Dao.getTemplateName().equals("BaseDao")){
            	templateEngine.processToFile(model, templateElement_Dao);
            }
            else {
